...
(function () {
	...
	function CreateInternalGame(gameName, options) {
		...
		if (jsContext == "worker") return WorkerCreateGame(gameName, options);else if (jsContext == "node") return NodeCreateGame(gameName, options);else // must be browser
	...
	GameProxy.prototype.userTurn = function () {
		...
		if (this.game) {
			...
			var promise = new Promise(function (resolve, reject) {
				...
				else promise2 = new Promise(function (resolve) {
					resolve();
				});
		...
		if (this.game) {
			...
			var promise = new Promise(function (resolve, reject) {
				...
				for (var o in optDefs) {
					if (typeof options[optDefs[o]] != "undefined") self.game[o] = options[optDefs[o]];
	...
	GameProxy.prototype.viewControl = function (command, options) {
		if (jsContext == "node") return Promise.reject(new Error("viewControl(): not supported in node.js"));
		if (!this.area && !this.iframe) return Promise.reject(new Error("viewControl(): match is not attached to DOM element"));
