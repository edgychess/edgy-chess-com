...
(function() {
	...
	Model.Game.cbDefine = function() {
		...
		return {
			...
			// ADAPTED for Edgy Chess
			pieceTypes: {

				0: {
					name: 'pawn-w',
					aspect: 'pawn',
					graph: this.cbPawnGraphEdgyChess(geometry,1),
					//graph: this.cbPawnGraphEdgyChessAlt(geometry,1),
					value: 1,
					abbrev: '',
					fenAbbrev: 'P',
					epCatch: true,
				...
				1: {
					...
					aspect: 'pawn',
					graph: this.cbPawnGraphEdgyChess(geometry,1),
					//graph: this.cbInitialPawnGraphEdgyChessAlt(geometry,1),
					value: 1,
					abbrev: '',
					fenAbbrev: 'P',
					initial: [{s:1,p:4},{s:1,p:9},{s:1,p:10},{s:1,p:11},{s:1,p:17},{s:1,p:25},{s:1,p:33},{s:1,p:40}],
					epTarget: true,
				},
				...
				3: {
					...
					abbrev: '',
					fenAbbrev: 'P',
					initial: [{s:-1,p:23},{s:-1,p:30},{s:-1,p:38},{s:-1,p:46},{s:-1,p:52},{s:-1,p:53},{s:-1,p:54},{s:-1,p:59}],
					epTarget: true,
				},
				...
				4: {
					...
					value: 2.9,
					abbrev: 'N',
					initial: [{s:1,p:2},{s:1,p:24},{s:-1,p:39},{s:-1,p:61}],
				},
				...
				5: {
					...
					value: 3.1,
					abbrev: 'B',
					initial: [{s:1,p:1},{s:1,p:16},{s:-1,p:47},{s:-1,p:62}],
				},
				...
				6: {
					...
					value: 5,
					abbrev: 'R',
					initial: [{s:1,p:3},{s:1,p:32},{s:-1,p:31},{s:-1,p:60}],
					castle: true,
				},
			...
			// ADAPTED for Edgy Chess: different fields
			promote: function(aGame,piece,move) {
				if(piece.t==1)
					return [0];
				else if(piece.t==3)
					return [2];
				else if(piece.t==0 && geometry.R(move.t)==7 && geometry.C(move.t)>=4 && geometry.C(move.t)<=7)
					return [4,5,6,7];
				else if(piece.t==0 && geometry.R(move.t)>=3 && geometry.R(move.t)<=7 && geometry.C(move.t)==7)
					return [4,5,6,7];
				else if(piece.t==2 && geometry.R(move.t)==0 && geometry.C(move.t)>=0 && geometry.C(move.t)<=3)
					return [4,5,6,7];
				else if(piece.t==2 && geometry.R(move.t)>=1 && geometry.R(move.t)<=4 && geometry.C(move.t)==0)
					return [4,5,6,7];
				return [];
			},

			// ADAPTED for Edgy Chess: different fiels
			castle: {
				"0/32": {k:[8,16],r:[24,16,8],n:"O-O-O"},
				"0/3": {k:[1,2],r:[2,1],n:"O-O"},
				"63/31": {k:[55,47],r:[39,47,55],n:"O-O-O"},
				"63/60": {k:[62,61],r:[61,62],n:"O-O"},
			},
			...
			evaluate: function(aGame,evalValues,material) {
				...
				var promoWhite = [4,4,4,4,2,2,2,2,4,4,4,4,2,1,1,1,4,4,4,4,2,1,0,0,4,4,4,4,2,1,0,9,4,3,3,3,3,2,1,9,4,2,1,1,1,1,0,9,4,2,1,0,0,0,0,9,4,2,1,0,9,9,9,9];
				var promoBlack = [9,9,9,9,0,1,2,4,9,0,0,0,0,1,2,4,9,0,1,1,1,1,2,4,9,0,1,2,2,2,2,4,9,0,1,2,4,4,4,4,0,0,1,2,4,4,4,4,1,1,1,2,4,4,4,4,2,2,2,2,4,4,4,4];
				var height=geometry.height;
				var pawns=material[1].byType[0],pawnsLength;
				if(pawns) {
					pawnsLength=pawns.length;
					for(var i=0;i<pawnsLength;i++)
						switch(promoWhite[pawns[i].p]) {
						case 0: distPromo[0]++; break;
						case 1: distPromo[1]++; break;
						case 2: distPromo[2]++; break;
						}
				}
			...
			importGame: function(initial,format,data) {
				initial.pieces.forEach(function(piece) {
					...
					if(piece.s==1 && (piece.t==0 || piece.t==1)) {
						if(geometry.R(piece.p)==0 && geometry.C(piece.p)==4 ||
						   geometry.R(piece.p)==1 && geometry.C(piece.p)>=1 && geometry.C(piece.p)<=3 ||
						   geometry.R(piece.p)>=2 && geometry.R(piece.p)<=4 && geometry.C(piece.p)==2 ||
						   geometry.R(piece.p)==5 && geometry.C(piece.p)==0) {
							if(piece.t==0) {
								piece.t=1;
								piece.m=false;
							}
					...
					if(piece.s==-1 && (piece.t==2 || piece.t==3)) {
						if(geometry.R(piece.p)==7 && geometry.C(piece.p)==3 ||
						   geometry.R(piece.p)==6 && geometry.C(piece.p)>=4 && geometry.C(piece.p)<=6 ||
						   geometry.R(piece.p)>=3 && geometry.R(piece.p)<=5 && geometry.C(piece.p)==6 ||
						   geometry.R(piece.p)==2 && geometry.C(piece.p)==7) {
							if(piece.t==2) {
								piece.t=3;
								piece.m=false;
							}
	...
	// NEW for Edgy Chess (pawns move like Legan Chess)
	Model.Game.cbPawnGraphEdgyChess = function(geometry,side,confine) {
		var $this=this;
		var graph={};
		for(var pos=0;pos<geometry.boardSize;pos++) {
			if(confine && !(pos in confine)){
			...
			var pos1=geometry.Graph(pos,[0,side]);
			if(pos1!=null && (!confine || (pos1 in confine)))
				directions.push($this.cbTypedArray([pos1 | $this.cbConstants.FLAG_CAPTURE]));

			var pos3=geometry.Graph(pos,[side,0]);
			if(pos3!=null && (!confine || (pos3 in confine)))
				directions.push($this.cbTypedArray([pos3 | $this.cbConstants.FLAG_CAPTURE]));

			var pos5=geometry.Graph(pos,[side,side]);
			if(pos5!=null && (!confine || (pos5 in confine)))
				directions.push($this.cbTypedArray([pos5 | $this.cbConstants.FLAG_MOVE]));
	...
	// ALTERNATIVE for Edgy Chess (classic pawn moves)
	Model.Game.cbPawnGraphEdgyChessAlt = function(geometry,side,confine) {
		var $this=this;
		var graph={};
		for(var pos=0;pos<geometry.boardSize;pos++) {
			if(confine && !(pos in confine)){
			...
			var pos3=geometry.Graph(pos,[side,0]);
			if(pos3!=null && (!confine || (pos3 in confine)))
				directions.push($this.cbTypedArray([pos3 | $this.cbConstants.FLAG_MOVE]));

			var pos5=geometry.Graph(pos,[side,side]);
			if(pos5!=null && (!confine || (pos5 in confine)))
				directions.push($this.cbTypedArray([pos5 | $this.cbConstants.FLAG_CAPTURE]));

			var pos6=geometry.Graph(pos,[side,-side]);
			if(pos6!=null && (!confine || (pos6 in confine)))
				directions.push($this.cbTypedArray([pos6 | $this.cbConstants.FLAG_CAPTURE]));

			var pos7=geometry.Graph(pos,[-side,side]);
			if(pos7!=null && (!confine || (pos7 in confine)))
				directions.push($this.cbTypedArray([pos7 | $this.cbConstants.FLAG_CAPTURE]));
	...
	// ALTERNATIVE for Edgy Chess (classic pawn moves)
	Model.Game.cbInitialPawnGraphEdgyChess = function(geometry,side,confine) {
		var $this=this;
		var graph={};
		for(var pos=0;pos<geometry.boardSize;pos++) {
			if(confine && !(pos in confine)){
			...
			var pos3=geometry.Graph(pos,[side,0]);
			if(pos3!=null && (!confine || (pos3 in confine))) {
				var direction=[pos3 | $this.cbConstants.FLAG_MOVE];
				var pos4=geometry.Graph(pos3,[side,0]);
				if(pos4!=null && (!confine || (pos4 in confine)))
					direction.push(pos4 | $this.cbConstants.FLAG_MOVE);
				directions.push($this.cbTypedArray(direction));
			}
